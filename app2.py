...
# -------------------------------------------------
# FEATURE ENGINEERING (Improved for CDR)
# -------------------------------------------------
X_fe = X.copy()
X_fe["GMO_x_ProbeTime"] = X["GMO"] * X["ProbeTime"]
X_fe["Poloxamer_x_ProbeTime"] = X["Poloxamer"] * X["ProbeTime"]
X_fe["GMO_x_Poloxamer"] = X["GMO"] * X["Poloxamer"]
X_fe["ProbeTime_sq"] = X["ProbeTime"] ** 2
X_fe["GMO_sq"] = X["GMO"] ** 2
...
def train_models():
    ...
    # Forward model (Particle Size + Entrapment)
    ...
    fwd_model.fit(X_tr, Y_tr[["ParticleSize", "Entrapment"]])

    # Improved CDR model (log transformed)
    y_cdr_log = np.log1p(Y_tr["CDR"])

    cdr_model = HistGradientBoostingRegressor(
        max_depth=6,
        learning_rate=0.05,
        max_iter=600,
        random_state=42
    )
    cdr_model.fit(X_fe.loc[X_tr.index], y_cdr_log)
    ...
    return fwd_model, cdr_model, bwd_model, X_tr, X_te, Y_tr, Y_te

fwd_model, cdr_model, bwd_model, X_train, X_test, Y_train, Y_test = train_models()

# -------------------------------------------------
# METRIC FUNCTIONS
# -------------------------------------------------
def classification_metrics(y_true, y_pred):
    thr = np.quantile(y_true, 0.6)
...
def compute_eer(y_true, y_score):
    ...
    return fpr, tpr, eer, idx, auc(fpr, tpr)
...
with tab1:
    ...
    user_X_fe["GMO_x_Poloxamer"] = gmo * pol
    user_X_fe["ProbeTime_sq"] = pt ** 2
    user_X_fe["GMO_sq"] = gmo ** 2

    ps_ee = fwd_model.predict(user_X)
    cdr = np.expm1(cdr_model.predict(user_X_fe))
    ...
    ps_ee_test = fwd_model.predict(X_test)
    cdr_test = np.expm1(cdr_model.predict(X_fe.loc[X_test.index]))
    ...
    target = st.selectbox("Select Output for ROC / CM", Y.columns)
    y_pred = cdr_test if target == "CDR" else ps_ee_test[:, Y.columns.get_loc(target)]
...
with tab2:
    ...
    st.dataframe(pd.DataFrame(
        rows, columns=["Input", "MSE", "Precision", "Recall", "F1"]
    ), use_container_width=True)
...
with tab3:
    ...
    grid = pd.DataFrame(
        [[g, p, t]
         for g in np.linspace(X.GMO.min(), X.GMO.max(), 12)
         for p in np.linspace(X.Poloxamer.min(), X.Poloxamer.max(), 12)
         for t in np.linspace(X.ProbeTime.min(), X.ProbeTime.max(), 12)],
        columns=X.columns
    )
    ...
    grid_fe["GMO_x_Poloxamer"] = grid["GMO"] * grid["Poloxamer"]
    grid_fe["ProbeTime_sq"] = grid["ProbeTime"] ** 2
    grid_fe["GMO_sq"] = grid["GMO"] ** 2

    ps_ee = fwd_model.predict(grid)
    grid["ParticleSize"] = ps_ee[:, 0]
    grid["Entrapment"] = ps_ee[:, 1]
    grid["CDR"] = np.expm1(cdr_model.predict(grid_fe))
